...
class spBlocknewsletter extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('SP Newsletter block');
	...
	public function getContent()
	{
		if (Tools::isSubmit('submitUpdate'))
		{
			...
			Configuration::updateValue('newsletterpopup', (bool)Tools::getValue('newsletterpopup'));
	...
	public function hookdisplayNewLetter2($params)
	{
		if (!isset($this->prepared) || !$this->prepared)
			$this->_prepareHook($params);
		$this->prepared = true;
		return $this->display(__FILE__, 'blocknewsletter2.tpl');
	}

	public function hookdisplayNewLetter3($params)
	{
		if (!isset($this->prepared) || !$this->prepared)
			$this->_prepareHook($params);
		$this->prepared = true;
		return $this->display(__FILE__, 'blocknewsletter3.tpl');
	}
	
	public function hookdisplayNewLetterPopup($params)
	{
		if (!Configuration::get('newsletterpopup')) return;
		if(isset($_COOKIE['sp_news_letter']))
			return	;
		if (!isset($this->prepared) || !$this->prepared)
			$this->_prepareHook($params);
		$this->prepared = true;
		return $this->display(__FILE__, 'blocknewsletterpopup.tpl');
	}
	...
	{
		$this->context->controller->addCSS($this->_path.'css/spblocknewsletter.css', 'all');
		$this->context->controller->addJS($this->_path.'jquery.cookie.min.js');
	}
	...
	public function getConfigFieldsValues()
	{
		return array(
			...
			'newsletterpopup' => Tools::getValue('newsletterpopup', Configuration::get('newsletterpopup')),
