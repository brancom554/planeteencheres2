...
class SpCountdownProductSliderClass extends ObjectModel
{
	public $id_spcountdownproductslider;
	public $title_module;
	public $short_desc;
	...
	public static $definition = array(
		'table' => 'spcountdownproductslider',
		'primary' => 'id_spcountdownproductslider',
		'multilang' => true,
	...
	{
		Shop::addTableAssociation ('spcountdownproductslider', array('type' => 'shop'));
	...
	public function getHigherPosition()
	{
		$sql = 'SELECT MAX(`ordering`)
				FROM `'._DB_PREFIX_.'spcountdownproductslider`';
	...
	{
		$sql = 'SELECT MAX(`id_spcountdownproductslider`)
				FROM `'._DB_PREFIX_.'spcountdownproductslider`';
		$id_spcountdownproductslider = DB::getInstance ()->getValue ($sql);
		return ( is_numeric ($id_spcountdownproductslider) )?$id_spcountdownproductslider:1;
	}
	...
	public static function getAssociatedIdsShop($id_module)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN `'._DB_PREFIX_.'spcountdownproductslider_shop` css ON (css.`id_spcountdownproductslider` = cs.`id_spcountdownproductslider`)
	...
	{
		$id_spcountdownproductslider = $this->id;
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE css.`id_spcountdownproductslider` = cs.`id_spcountdownproductslider` AND css.`id_shop` = '.(int)$id_shop
		);

		if ((int)$max == (int)$id_spcountdownproductslider)
			return true;
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT cs.`ordering` as ordering, cs.`id_spcountdownproductslider` as id_spcountdownproductslider
			FROM `'._DB_PREFIX_.'spcountdownproductslider` cs
			LEFT JOIN `'._DB_PREFIX_.'spcountdownproductslider_shop` css ON (css.`id_spcountdownproductslider` = cs.`id_spcountdownproductslider`)
		...
		foreach ($rows as $row)
		{
			$customs = new spcountdownproductsliderClass($row['id_spcountdownproductslider']);
			--$customs->ordering;
			$customs->update();
			unset($customs);
		}
