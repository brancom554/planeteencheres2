...
class SpDeal extends Module
{
	protected $categories = array();
	protected $error = false;
	private $html;
	private $default_hook = array( 'displayDeal', 'displayDeal2', 'displayDeal3' );
	...
	public function installFixtures()
	{
		...
		$datas = array(
			array(
				'id_splistingtabs' => 1,
				'title_module' => 'Flash Deal',
				'display_title_module' => '1',
				'moduleclass_sfx' => 'deal-1',
				'active' => 1,
			...
			array(
				'id_splistingtabs' => 2,
				'title_module' => 'Recommend',
				'display_title_module' => '1',
				'moduleclass_sfx' => 'deal-2',
				'active' => 1,
	...
	public function hookDisplayDeal()
	{
		$smarty = $this->context->smarty;
		$smarty_cache_id = $this->getCacheId ('spdeal_displayDeal');
		
			$list = $this->getItemInHook ('displayDeal');
			if (empty( $list))
				return;
				foreach($list as $_list){
					if(isset($_list->products)){
						$smarty->assign (array('check_n' => '1'));
					}
				}
			$smarty->assign (array(
				'list' => $list,
	...
	public function hookDisplayDeal2()
	{
		$smarty = $this->context->smarty;
		$smarty_cache_id = $this->getCacheId ('spdeal_displayDeal2');
		
			$list = $this->getItemInHook ('displayDeal2');
			if (empty( $list))
				return;
				foreach($list as $_list){
					if(isset($_list->products)){
	...
	public function hookDisplayDeal3()
	{
		$smarty = $this->context->smarty;
		$smarty_cache_id = $this->getCacheId ('spdeal_displayDeal3');
		
			$list = $this->getItemInHook ('displayDeal3');
			if (empty( $list))
				return;
				foreach($list as $_list){
					if(isset($_list->products)){
		...
		return $this->display (__FILE__, 'default3.tpl');
	}
