...
class SpExtraSliderClass extends ObjectModel
{
	public $id_spextraslider;
	public $title_module;
	public $short_desc;
	...
	{
		Shop::addTableAssociation ('spextraslider', array('type' => 'shop'));
	...
	public function getHigherPosition()
	{
		$sql = 'SELECT MAX(`ordering`)
				FROM `'._DB_PREFIX_.'spextraslider`';
	...
	{
		$sql = 'SELECT MAX(`id_spextraslider`)
				FROM `'._DB_PREFIX_.'spextraslider`';
		$id_spextraslider = DB::getInstance ()->getValue ($sql);
		return ( is_numeric ($id_spextraslider) )?$id_spextraslider:1;
	}
	...
	public static function getAssociatedIdsShop($id_module)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN `'._DB_PREFIX_.'spextraslider_shop` css ON (css.`id_spextraslider` = cs.`id_spextraslider`)
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE css.`id_spextraslider` = cs.`id_spextraslider` AND css.`id_shop` = '.(int)$id_shop
		);

		if ((int)$max == (int)$id_spextraslider)
			return true;
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT cs.`ordering` as ordering, cs.`id_spextraslider` as id_spextraslider
			FROM `'._DB_PREFIX_.'spextraslider` cs
			LEFT JOIN `'._DB_PREFIX_.'spextraslider_shop` css ON (css.`id_spextraslider` = cs.`id_spextraslider`)
		...
		foreach ($rows as $row)
		{
			$customs = new spextrasliderClass($row['id_spextraslider']);
			--$customs->ordering;
			$customs->update();
			unset($customs);
		}
