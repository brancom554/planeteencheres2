...
class SpSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public $position;
	public $id_shop;
	public $id_sphomeslider_groups;

	/**
	 * @see ObjectModel::$definition
	 */
	public static $definition = array(
		'table' => 'sphomeslider_slides',
		'primary' => 'id_sphomeslider_slides',
		'multilang' => true,
		'fields' => array(
			'active' =>			array('type' => self::TYPE_BOOL, 'validate' => 'isBool', 'required' => true),
			'position' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
			'id_sphomeslider_groups' =>		array('type' => self::TYPE_INT, 'validate' => 'isunsignedInt', 'required' => true),
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'sphomeslider` (`id_shop`, `id_sphomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'sphomeslider`
			WHERE `id_sphomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM `'._DB_PREFIX_.'sphomeslider_slides` hss, `'._DB_PREFIX_.'sphomeslider` hs
			WHERE hss.`id_sphomeslider_slides` = hs.`id_sphomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_sphomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'sphomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'sphomeslider` hs ON (hss.`id_sphomeslider_slides` = hs.`id_sphomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new SpSlide($row['id_slide']);
