...
class SpMostViewedClass extends ObjectModel
{
	public $id_spmostviewed;
	public $title_module;
	public $short_desc;
	...
	{
		Shop::addTableAssociation ('spmostviewed', array('type' => 'shop'));
	...
	{
		$sql = 'SELECT MAX(`id_spmostviewed`)
				FROM `'._DB_PREFIX_.'spmostviewed`';
		$id_spmostviewed = DB::getInstance ()->getValue ($sql);
		return ( is_numeric ($id_spmostviewed) )?$id_spmostviewed:1;
	}
	...
	public static function getAssociatedIdsShop($id_module)
	{
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN `'._DB_PREFIX_.'spmostviewed_shop` css ON (css.`id_spmostviewed` = cs.`id_spmostviewed`)
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE css.`id_spmostviewed` = cs.`id_spmostviewed` AND css.`id_shop` = '.(int)$id_shop
		);

		if ((int)$max == (int)$id_spmostviewed)
			return true;
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT cs.`ordering` as ordering, cs.`id_spmostviewed` as id_spmostviewed
			FROM `'._DB_PREFIX_.'spmostviewed` cs
			LEFT JOIN `'._DB_PREFIX_.'spmostviewed_shop` css ON (css.`id_spmostviewed` = cs.`id_spmostviewed`)
		...
		foreach ($rows as $row)
		{
			$customs = new spmostviewedClass($row['id_spmostviewed']);
			--$customs->ordering;
			$customs->update();
			unset($customs);
		}
