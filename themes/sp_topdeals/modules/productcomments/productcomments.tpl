  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
       ...
       <h4 class="title"><i class="fa fa-comments" aria-hidden="true"></i> {l s='Write your review' d='Shop.Theme.Actions'}</h4>
      </div>
	<div class="modal-body">
		<div id="comment-form-wrap">
			...
			<div class="comment-form-wrap_content col-xs-12">
				...
				<label for="comment_title">{l s='Title for your review' d='Shop.Theme.Actions'} <sup class="required">*</sup></label>
				<input id="comment_title" class=" form-control" name="title" type="text" value=""/>

				<label for="content">{l s='Your review' d='Shop.Theme.Actions'} <sup class="required">*</sup></label>
				<textarea id="content" name="content" class=" form-control"></textarea>

				{if $allow_guests == true && !$logged}
				<label>{l s='Your name' d='Shop.Theme.Actions'} <sup class="required">*</sup></label>
				...
				<div id="comment-form-wrap_footer">
					...
					<p class="fl"><sup class="required">*</sup> <small>{l s='Required fields' d='Shop.Theme.Actions'}</small></p>
					<p class="fr">
						<a class="btn btn-info" href="javascript:void(0)" id="submitNewMessage" onclick="fnSubmitNewMessage()"><i id="iloading-icon" class="fa fa-circle-o-notch fa-spin fa-fw" style="display: none;"></i> {l s='Send' d='Shop.Theme.Actions'}</a>
					</p>
					<div style="clear: both;"></div>
				</div>
			</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
       ...
       <h4 class="title"><i class="fa fa-comments" aria-hidden="true"></i> {l s='New comment' d='Shop.Theme.Actions'}</h4>
      </div>
		<div class="modal-body">
			<div id="comment-form-wrap_success"  >
				<p class="alert alert-success">
				{if $is_comments_moderate}
					{l s="You are sent a comment success. The administrator will review and approve your comment. Thank you!" d='Shop.Theme.Actions'}
				{else}
					{l s="You are sent a comment success. Thank you!" d='Shop.Theme.Actions'}
				{/if}	
				</p>
...
<div id="idTab5" style="margin-top: 2rem;">
	...
	{if $comments}
					...
					<div class="text">
						...
						<div>
							{if $comment.total_advice > 0}
								<div class="div-people-like">{l s='%1$d out of %2$d people found this review useful.' sprintf=[$comment.total_useful,$comment.total_advice] d='Shop.Theme.Actions'}</div>
							{/if}
							{if $logged}
								<div class="text-bottom">
								{if !$comment.customer_advice}
								<span class="usefulness_btn" onclick="iFnLike(this)" data-is-usefull="1" data-id-product-comment="{$comment.id_product_comment}">{l s='Like' d='Shop.Theme.Actions'}</span>
								<span class="usefulness_btn" onclick="iFnLike(this)" data-is-usefull="0" data-id-product-comment="{$comment.id_product_comment}">{l s='Dislike' d='Shop.Theme.Actions'}</span>
								{/if}
								{if !$comment.customer_report}
								<span class="report_btn  pull-right" onclick="iFnReport(this)" data-id-product-comment="{$comment.id_product_comment}">{l s='Report' d='Shop.Theme.Actions'}</span>
								{/if}
								</div>
							{/if}
						</div>
		...
		<p class="align_center ialign_center">
			<a class="btn" id="new_comment_tab_btn"  href="#" data-toggle="modal" data-target="#productcomment-modal">{l s='Write your review' d='Shop.Theme.Actions'}</a>
		</p>
        {/if}
	{else}
		{if (!$too_early AND ($logged OR $allow_guests))}
		<p class="align_center">
			<a id="new_comment_tab_btn" class="open-comment-form" href="#" data-toggle="modal" data-target="#productcomment-modal">{l s='Be the first to write your review' d='Shop.Theme.Actions'} !</a>
		</p>
		{else}
		<p class="align_center hide">{l s='No customer reviews for the moment.' d='Shop.Theme.Actions'}</p>
		{/if}
	{/if}	
	</div>
</div>
