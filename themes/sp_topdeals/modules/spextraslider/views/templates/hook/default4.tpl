...
{if isset($list) && !empty($list)}
    {foreach from=$list item=items}
        ...
        <div class="moduletable  {$moduleclass_sfx|escape:'html':'UTF-8'}">
                ...
                {if isset($_list) && $_list}
	                <div id="{$tag_id|escape:'html':'UTF-8'}"
	                    ...
	                    <div class="extraslider-inner product-listing" data-effect="{$items->params.effect|escape:'html':'UTF-8'}">
	                        ...
	                        {foreach $_list as $sp_product}
	                            {counter name=count}
	                            {if $count % $nb_rows == 1 || $nb_rows == 1}
	                                <div class="item">
	                            {/if}
								<article class="product-miniature js-product-miniature" data-id-product="{$sp_product.id_product}" data-id-product-attribute="{$sp_product.id_product_attribute}" itemscope itemtype="http://schema.org/Product">		
		                            <div class="item-wrap {$items->params.layout|escape:'html':'UTF-8'}">
		                                <div class="product-container">
		                                	<div class="left-block">
												<div class="product-image">
													{block name='product_thumbnail'}
														<a href="{$sp_product.link|escape:'html':'UTF-8'}" title="{$sp_product.name|escape:'html':'UTF-8'}">
															{assign var="imageSize" value=($items->params.image_size != 'none') ? $items->params.image_size : 'home_default'}
															{if $sp_product.cover.bySize.$imageSize.url}
																<img src = "{$sp_product.cover.bySize.$imageSize.url}" alt = "{$sp_product.cover.legend}" data-full-size-image-url = "{$sp_product.cover.large.url}">
															{else}
																{assign var="src" value={$link->getImageLink($sp_product.link_rewrite, $sp_product.id_image, $items->params.image_size)|escape:'html':'UTF-8'}}
																<img src="{$src|escape:'html':'UTF-8'}"
																	 alt="{$sp_product.legend|escape:'html':'UTF-8'}"/>
															{/if}
															{if isset($SP_secondimg)}
																{hook h="displaySecondImage" id_product=$product.id_product link_rewrite=$sp_product.link_rewrite}
															{/if}
														</a>
													{/block}
													{if $items->params.display_new || $items->params.display_sale }	
													  	{block name='product_flags'}
															<div class="product-flags">
															  	{foreach from=$sp_product.flags item=flag key = k}
																	{if $k=='new'}	
													...
													{/if}
												  	...
												  	<!--{if $sp_product.discount_type === 'percentage'}
													  <span class="discount-percentage">{$sp_product.discount_percentage}</span>
													{/if}-->
												...
												<div class="product-info">
													{if $items->params.display_name == 1}
													  	{block name='product_name'}
															<h5 class="product-title" itemprop="name"><a href="{$sp_product.link}">{$sp_product.name}</a></h5>
													  	{/block}
													{/if}

													{if $items->params.display_variant}
													  	{block name='product_variants'}
															{if $sp_product.main_variants}
															  	{include file='catalog/_partials/variant-links.tpl' variants=$sp_product.main_variants}
															{/if}
													  	{/block}
													{/if}  
													{if $items->params.display_availability}
													  	{block name='product_availability'}
															{if $sp_product.show_availability}
															  	{* availability may take the values "available" or "unavailable" *}
															  	<span class='product-availability {$sp_product.availability}'>{$sp_product.availability_message}</span>
															{/if}
													  	{/block}
													{/if}
													{if $items->params.display_description}
													  	{block name='product_description_short'}
															<div class="product-description" itemprop="description">{$sp_product.description_short nofilter}</div>
													  	{/block}
													{/if}
													  	...
													  	{block name='product_price_and_shipping'}
															<div class="product-price-and-shipping" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
	                											...
	                											<meta itemprop="priceCurrency" content="{$currency.iso_code}">
															  	{if $sp_product.has_discount}
																	{hook h='displayProductPriceBlock' product=$sp_product type="old_price"}
																	<span class="regular-price">{$sp_product.regular_price}</span>
															  	{/if}
															  	<span itemprop="price" content="{$sp_product.price_amount}" class="price">{$sp_product.price}</span>
															  	{hook h='displayProductPriceBlock' product=$sp_product type="before_price"}
															  	{hook h='displayProductPriceBlock' product=$sp_product type='unit_price'}
															  	{hook h='displayProductPriceBlock' product=$sp_product type='weight'}
															</div>
													  	{/block}
													{/if}
							                        {if $items->params.display_addtocart}
							                          <div class="cart_content">
							                            {if $sp_product.availability == 'available'}
							                                <a href="#" class="ajax-add-to-cart product-btn cart-button" data-id-product="{$sp_product.id_product}" data-minimal-quantity="1" title = "{l s='Add to cart' d='Shop.Theme.Actions'}">
							                                  <span class="text">{l s='Add to cart' d='Shop.Theme.Actions'}</span>
							                                </a>
							                              {elseif $sp_product.availability == 'last_remaining_items'}
							                                <a href="#" class="ajax-add-to-cart product-btn cart-button" data-id-product="{$sp_product.id_product}" data-minimal-quantity="1" title = "{l s='Add to cart' d='Shop.Theme.Actions'}">
							                                  <span class="text">{l s='Add to cart' d='Shop.Theme.Actions'}</span>
							                                </a>
							                              {else}
							                                <span class="no-product cart-button" data-id-product="{$sp_product.id_product}" data-minimal-quantity="1" title = "{l s='Add to cart' d='Shop.Theme.Actions'}">
							                                  <span class="text">{l s='Add to cart' d='Shop.Theme.Actions'}</span>
							                                </span>
							                            {/if}
							                          </div>
            ...
            {else}
                {l s='Has no content to show!' d='Shop.Theme.Actions'}
            {/if}
        </div>
    {/foreach}
{/if}
...
<script>
    ...
    $(document).ready(function(){
        $(".button-close").click(function(){
            if($('.sp-extra-1.special').hasClass('active-special')){
                $('.sp-extra-1.special').removeClass('active-special');
            }else{
                $('.sp-extra-1.special').removeClass('active-special');
                $('.sp-extra-1.special').addClass('active-special');
            }
        });
